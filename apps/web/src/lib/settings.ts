...
/** app_settings が空でも動くための既定値 (シードSQLと同値) */
export const DEFAULT_SETTINGS: AppSettings = {
  volumetric_divisor: 6000,
  dummy_weight_values: [0, 1, 999, 1000, 9999],
  amazon_domestic_shipping_jpy: 0,
  shipping_note_text:
...
/** 粗利計算・補正・コンプラ検査・市場別出品に必要な参照データ一式 */
...
export async function loadEnrichContext(): Promise<EnrichContext> {
  ...
  const [settings, markets, cats, rates, kws] = await Promise.all([
    loadSettings(),
  ...
  if (markets.error) throw new Error(`markets取得失敗: ${markets.error.message}`);
